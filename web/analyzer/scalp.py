...
# to parse the timezone
def convert_zone2seconds(zone):
    # +-0000 -> +hhmm
    zone = zone.strip()
        ...
        if attack_type in regs:
            ...
            for _hash in regs[attack_type]:
                if _hash not in done:
                    ...
                    attack = table[_hash]
                    cur_line = exp_line[5]
                    if "password2=hpass1" in cur_line:
                        lst_attacks.append(code_std['csrf'])
                    if preferences['encodings']:
                        cur_line = decode_attempt(cur_line)
                    if attack[0].search(cur_line):
                        if attack[1] not in array[attack_type]:
                        ...
                        #print cur_line
                        #print attack[3]
                        lst_attacks.append(code_std[str(attack_type)])
                        if preferences['exhaustive']:
                            break
                        else:
                            return
    if not lst_attacks:
        return [code_std['safe']]
    return lst_attacks[0]
...
def pipe_analyzer(filters, preferences = []):
    global table
    ...
    the_file_name = str(time.strftime("%Y%m%d-%H%M%S")) + ".log"
    with open(the_file_name, 'w') as the_file:
        while(1):
            ...
            if c_reg.match(line):
                ...
                if len(url) > 1 and method in ('GET','POST','HEAD','PUT','PUSH','OPTIONS'):
                    attacks_found = analyzer4pipe([(ip,name,date,ext,method,url,response,byte,referrer,agent),regs,flag, preferences, line])

            d = datetime.strptime(str(date).strip(),'%d/%b/%Y:%H:%M:%S')
            ...
            # print data_log
            # print "|%s|" % ext
            # print "|%s|" % data_log[4][:-1]
            
            zd = convert_zone2seconds(data_log[4][:-1])
            #zd = convert_zone2seconds("+0105")
            ...
            if line.find("HTTP/") != -1:
                http_v = line[line.find("HTTP/"):line.find("HTTP/") + 8]
            else:
                http_v = "HTTP/non"


            str_log = ','.join(attacks_found) + " "
            str_log += str(ip)[:-3] + " " + ts + " " + str(method) + " "
            str_log += "\"" + str(url)[1:] + "\" " + str(response) + " "
            str_log += cl + " \"" + http_v + "\" "
            str_log += referrer[1:-1]

            the_file.write(str_log+"\n")

            if attacks_found[0] == '0':
                print str_log
            else:
                print str_log
            # print "###########################################################"
...
def help():
    ...
    print("   --pipe      |-z   in order to enable pipe mode for real time analysis")
    ...
    print("example for pipe:\n\nusage: [pipe data stream from another program] | python -u scalp.py [--pipe|-z] [--filters|-f filter_file]\n")
...
def main(argc, argv):
    ...
    if argc < 2 or argv[1] == "--help":
        ...
        if s in ("--pipe","-z"):
            ...
            fill_replace_dict()
            preferences['encodings'] = True
            pipe_analyzer(filters, preferences)
        else:
